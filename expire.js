...
function scheduleRevocation(client) {
  const command = `echo "cd /etc/openvpn/easy-rsa && ./easyrsa revoke ${client} && ./easyrsa gen-crl && service openvpn restart" | at now + 24 hours`;
  try {
    execSync(command);
    console.log(`Scheduled revocation for client ${client}`);
  } catch (err) {
    console.error('Error scheduling revocation:', err);
  }
}
